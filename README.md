...
- **Подтверждение для команд среднего и высокого риска** (замки, багажник, люк, шторка, обогрев)
...
Управление включается в настройках интеграции (флажок «Включить удаленное управление»).

**Обязательные условия:**
- ПИН из личного кабинета GWM (6 цифр) — сохраняется в настройках интеграции или передаётся в каждом вызове сервиса
- Команды среднего и высокого риска (замки, багажник, люк, шторка, обогрев) требуют `confirm: true` при вызове через сервис

**Кнопки в интерфейсе HA:**
- Появляются, если ПИН сохранён в настройках и включено удалённое управление
- Отображаются все доступные команды (ограничений по уровню риска нет)
...
Команды `flash_lights`, `horn` и `flash_and_horn` не требуют подтверждения — подходят для экстренных сценариев.
